...
class StatementParser:
    ...
    def parse_method_body(self, lines: List[str], start_idx: int) -> Tuple[List[str], int]:
        ...
        while i < len(lines):
            ...
            if '"""' in stripped_line:
                if not in_multiline_comment:
                    ...
                    if stripped_line.startswith('"""') and stripped_line.endswith('"""') and len(stripped_line) > 6:
                        # Single line with triple quotes: """This is a comment"""
                    ...
                    if stripped_line.endswith('"""'):
                        ...
                        # """ appears in middle, ending comment
            ...
            # If we're inside a multi-line comment, treat as comment content
            ...
            # Preserve Java-style comments as-is
            if stripped_line.startswith('//'):
                body.append(stripped_line)
                i += 1
                continue
            
            # Handle Python-style single-line comments
            if stripped_line.startswith('#'):
            ...
            # Handle lines with inline comments (both // and #)
            comment_found = False
            java_line = ""
            
            # Check for Java-style inline comments first
            if '//' in stripped_line:
                code_part, comment_part = stripped_line.split('//', 1)
            ...
            # Check for Python-style inline comments
    ...
    def _is_collection_operation(self, statement: str) -> bool:
        ...
        patterns = [
            ...
            r'remove\s+index\s+.+\s+from\s+\w+',  # New: remove by index
            r'remove\s+value\s+.+\s+from\s+\w+',  # New: explicit remove by value
            r'clear\s+\w+',
            r'size\s+of\s+\w+',
            r'length\s+of\s+\w+',
            r'count\s+of\s+\w+',
    ...
    def _convert_collection_operation_expression(self, statement: str) -> str:
        ...
        # Remove from collection - Enhanced with index/value distinction
        if re.match(r'remove\s+index\s+.+\s+from\s+\w+', stmt_lower):
            # Remove by index: "remove index 2 from numbers" -> "numbers.remove(2)"
            ...
            # Remove by value (explicit): "remove value 5 from numbers" -> "numbers.remove(Integer.valueOf(5))"
            match = re.match(r'remove\s+value\s+(.+?)\s+from\s+(\w+)', statement, re.IGNORECASE)
            if match:
                value, collection = match.groups()
                value = value.strip()
                # For Integer collections, wrap non-quoted numbers with Integer.valueOf()
            ...
            # Default remove behavior - try to be smart about it
            match = re.match(r'remove\s+(.+?)\s+from\s+(\w+)', statement, re.IGNORECASE)
            if match:
                item, collection = match.groups()
                item = item.strip()
                # If it's a quoted string, remove as-is
                if item.startswith('"') and item.endswith('"'):
                    return f"{collection}.remove({item})"
                # If it's a number without quotes, assume remove by value for Integer collections
                elif item.isdigit():
                    return f"{collection}.remove(Integer.valueOf({item}))"
                # Otherwise, remove as-is (could be a variable)
                else:
                    return f"{collection}.remove({item})"
