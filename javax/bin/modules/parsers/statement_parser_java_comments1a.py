    ...
    def parse_method_body(self, lines: List[str], start_idx: int) -> Tuple[List[str], int]:
        ...
        while i < len(lines):
            ...
            if '//' in stripped_line:
                ...
                if code_part:
                    # Convert the code part and add the comment
                    java_line = self.convert_statement_to_java(code_part)
                    if comment_part:
                        # Add comment after the complete Java statement
            ...
            if any(line.endswith('{') for line in [java_line] if 'java_line' in locals()):
