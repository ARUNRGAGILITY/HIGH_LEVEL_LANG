...
def test_parser():
    """Test the parser with enhanced syntax including synonyms"""
    
    # Test with different template synonyms and their corresponding section names
    test_cases = [
    ...
    for synonym, header, vars_section, methods_section in test_cases:
        print(f"\n{'='*50}")
        print(f"Testing with '{synonym}' keyword and consistent sections:")
        print(f"{'='*50}")
    ...
    instance methods:
        * addGrade(string course, double grade)
            ...
            grades.add(grade)
            updateGPA()
            print Added grade {{grade}} for course {{course}}
...
main
    print Welcome to {{Student.getSchoolName()}}
...
'''
        ...
        try:
            ...
            print(f"✅ Successfully parsed with '{synonym}' keyword and '{vars_section}'/'{methods_section}' sections")
            print("Generated Java Code (first 20 lines):")
            print("-" * 40)
            ...
            print(f"❌ Error with '{synonym}' keyword: {e}")
    
    # Test backward compatibility - old template syntax should still work
    print(f"\n{'='*50}")
    print("Testing backward compatibility with mixed old/new syntax:")
    print(f"{'='*50}")
    
    mixed_sample = '''blueprint Vehicle
    template vars:
        * vehicleCount as int with 0
...
main
    create car as Vehicle with "Toyota", "Camry", 2023
    print Vehicle info: {{car.getInfo()}}
    print Total vehicles: {{Vehicle.getVehicleCount()}}
'''
    
    parser = PseudoJavaParser()
    try:
        java_code = parser.parse_program(mixed_sample)
        print("✅ Successfully parsed mixed syntax (blueprint + template vars + blueprint methods)")
        print("Generated Java Code:")
        print("-" * 40)
        print(java_code)
    except Exception as e:
        print(f"❌ Error with mixed syntax: {e}")
    
    # Test all possible combinations
    print(f"\n{'='*50}")
    print("Testing all supported static section names:")
    print(f"{'='*50}")
    
    static_sections = [
        "template vars", "template methods",
        "blueprint vars", "blueprint methods", 
        "design vars", "design methods",
        "class vars", "class methods",
        "static vars", "static methods"
    ]
    
    print("Supported static variable sections:", [s for s in static_sections if 'vars' in s])
    print("Supported static method sections:", [s for s in static_sections if 'methods' in s])
...
main
    ...
    planning to change this to optimized constructor
