...
class PseudoJavaParser:
    def __init__(self):
        ...
        # Template synonyms - all equivalent to "template"
    ...
    def _parse_template(self, lines: List[str], start_idx: int) -> Tuple[Template, int]:
        """Parse a template definition (supports all synonyms)"""
        ...
        for synonym in self.template_synonyms:
            if template_line.lower().startswith(synonym.lower()):
    ...
    def _parse_method(self, lines: List[str], start_idx: int, is_static: bool = False, is_constructor: bool = False, template: Template = None) -> Tuple[Optional[Method], int]:
        ...
        if is_constructor:
            # Check for new simplified constructor syntax (just parameters, no class name)
            if ':' in line and not line.endswith(')'):
                # New syntax: parameters followed by colon (e.g., "name:" or "name, model:")
                ...
                # Original constructor syntax: * ClassName(params)
        ...
        if not (is_constructor and ':' in lines[start_idx].strip() and not lines[start_idx].strip().endswith(')')):
...
def test_parser():
    """Test the parser with enhanced syntax including synonyms and simplified constructors"""
    
    # Test simplified constructor syntax
    print(f"\n{'='*60}")
    print("Testing SIMPLIFIED CONSTRUCTOR SYNTAX:")
    print(f"{'='*60}")
    
    simple_constructor_test = '''template Car
    template vars:
    ...
    constructor:
        * name:
            totalCars++
            print f"Creating car: {name}"
            
        * name, model:
            totalCars++
            print f"Creating {model} named {name}"
...
main
    create car1 as Car with "Lightning"
    create car2 as Car with "Herbie", "Beetle"
    create car3 as Car with "Kitt", "Trans Am", 1982
    print f"Total cars created: {Car.totalCars}"
'''
    
    parser = PseudoJavaParser()
    try:
        java_code = parser.parse_program(simple_constructor_test)
        print("✅ Successfully parsed simplified constructor syntax")
        print("Generated Java Code:")
        print("-" * 50)
        print(java_code)
    except Exception as e:
        print(f"❌ Error with simplified constructors: {e}")
    
    # Test backward compatibility with original constructor syntax
    print(f"\n{'='*60}")
    print("Testing BACKWARD COMPATIBILITY - Original Constructor Syntax:")
    print(f"{'='*60}")
    
    original_constructor_test = '''template Student
    template vars:
    ...
    constructor:
        * Student(studentId, name):
            ...
            this.gpa = 0.0
            totalStudents++
            print f"Student {name} enrolled with ID {studentId}"
...
'''
    ...
    try:
        java_code = parser.parse_program(original_constructor_test)
        print("✅ Successfully parsed original constructor syntax")
        print("Generated Java Code:")
        print("-" * 50)
        print(java_code)
    except Exception as e:
        print(f"❌ Error with original constructors: {e}")
    
    # Test mixed constructor syntaxes in one class
    print(f"\n{'='*60}")
    print("Testing MIXED CONSTRUCTOR SYNTAXES:")
    print(f"{'='*60}")
    
    mixed_constructor_test = '''blueprint Vehicle
    blueprint vars:
        * totalVehicles as int with 0
    ...
    constructor:
        ...
        * Vehicle(make, model):
            ...
            this.color = "White"
            totalVehicles++
            print f"Standard vehicle: {make} {model}"
            
        * make, model, year:
            totalVehicles++
            this.color = "Black"
            if year > 2020:
                print "This is a modern vehicle"

main
    create v1 as Vehicle with "Toyota"
    create v2 as Vehicle with "Honda", "Civic"
    create v3 as Vehicle with "Tesla", "Model 3", 2023
    print f"Total vehicles: {Vehicle.totalVehicles}"
'''
    
    try:
        java_code = parser.parse_program(mixed_constructor_test)
        print("✅ Successfully parsed mixed constructor syntaxes")
        print("Generated Java Code:")
        print("-" * 50)
        print(java_code)
    except Exception as e:
        print(f"❌ Error with mixed constructors: {e}")
    
    # Test with different template synonyms and consistent sections
    test_cases = [
        ("template", "template Student", "template vars", "template methods"),
        ("blueprint", "blueprint Student", "blueprint vars", "blueprint methods"),
        ("design", "design Student", "design vars", "design methods"),
        ("class", "class Student", "class vars", "class methods")
    ]
    
    for synonym, header, vars_section, methods_section in test_cases:
        print(f"\n{'='*50}")
        print(f"Testing '{synonym}' with simplified constructors:")
        print(f"{'='*50}")
        
        sample_code = f'''{header}
    {vars_section}:
        * totalStudents as int with 0
    ...
    instance methods:
        * getInfo() returns string
            return name + " (" + studentId + ")"

main
    create student as {synonym.capitalize()} with "S001", "Alice"
    print {{student.getInfo()}}
'''
        
        try:
            java_code = parser.parse_program(sample_code)
            print(f"✅ Successfully parsed '{synonym}' with simplified constructors")
            print("Generated Java Code (first 15 lines):")
            print("-" * 40)
            ...
            print(f"❌ Error with '{synonym}' simplified constructors: {e}")
